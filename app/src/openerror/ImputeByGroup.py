# Source: https://github.com/openerror/PandasTransform/blob/main/src/ImputeByGroup.py
...
class ImputeByGroup:
    ...
    # the columns on which to pd.DataFrame.groupby, if any
    groupby_col: Iterable[str] = None
    # throw KeyError at unseen groupby_col values in .transform?
    key_error_on_unseen: bool = True
    ...
    def __post_init__(self):
        ...
        self.groupby_col = [self.groupby_col] if isinstance(
            self.groupby_col, str) else self.groupby_col
    ...
    def _prepare_imputation_values(self, standin: Any):
        ...
        if not self.key_error_on_unseen:
            self.imputation_values = defaultdict(
                lambda: standin, self.imputation_values)
    ...
    def transform(self, X: pd.DataFrame) -> Union[pd.Series, pd.DataFrame]:
        ...
        if self.groupby_col:
            ...
            imputed_target_col = X.loc[na_mask].apply(
                lambda row: self.imputation_values[tuple(
                    row.loc[self.groupby_col])],
                axis=1
            )
        else:
            imputed_target_col = X.loc[na_mask, self.target_col].fillna(
                self.imputation_values)
...
class ImputeNumericalByGroup(ImputeByGroup, BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        assert isinstance(X, pd.DataFrame)
        if self.groupby_col:
            self.imputation_values = X.groupby(self.groupby_col)[
                self.target_col].median()
            self._prepare_imputation_values(
                standin=X[self.target_col].median())
        else:
...
class ImputeCategoricalByGroup(ImputeByGroup, BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        if self.groupby_col:
            ...
            self._prepare_imputation_values(
                standin=X[self.target_col].mode().min())
        else:
